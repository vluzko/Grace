        ...
        let bytecode = match self.data {
            Stmt::FunctionDecStmt {ref name, ref args, ref block, ..} => {
                ...
                for name in self.get_true_declarations(context) {
                    let local_var_type = context.get_type(block.scope, &name);
            ...
            Stmt::AssignmentStmt {ref name, ref expression, ..} => {
                let identifier_bytecode = name.generate_bytecode(context);
                let expression_bytecode = expression.generate_bytecode(context);
	        ...
	        Stmt::IfStmt {ref condition, ref block, ref else_block, ..} => {
	            let condition_bytecode = condition.generate_bytecode(context);
	            let main_block_bytecode = block.generate_bytecode(context);
	            let else_block_bytecode = match else_block {
	                Some(content) => content.generate_bytecode(context),
	                None => "".to_string()
	            };
            ...
            Stmt::ReturnStmt (ref value) => {
                value.generate_bytecode(context)
            },
            ...
            Stmt::WhileStmt {ref condition, ref block, ..} => {
                let block_bytecode = block.generate_bytecode(context);
                let condition_bytecode = condition.generate_bytecode(context);
            ...
            Stmt::StructDec{ref name, ref fields} => {
                ...
                let size = context.type_map.get(&self.id).unwrap().size();
    ...
    fn generate_bytecode(&self, context: &Context2) -> String {
        let bytecode_rep = match self.data {
            Expr::ComparisonExpr {ref operator, ref left, ref right, ..} => {
                let first = left.generate_bytecode(context);
                let second = right.generate_bytecode(context);
                let operator = operator.generate_bytecode(context);
                format!("{}\n{}\n{}", first, second, operator)
            },
            Expr::BinaryExpr {ref operator, ref left, ref right, ..} => {
                let return_type = typing::choose_return_type(context.type_map.get(&self.id).unwrap());
            ...
            Expr::UnaryExpr {ref operator, ref operand, ..} => {
                let operand_type = typing::choose_return_type(&context.type_map.get(&operand.id).unwrap().clone());
                let operator_bytecode = operator.generate_typed_bytecode(&operand_type);
                let operand_bytecode = operand.generate_bytecode(context);
                format!("{}\n{}", operand_bytecode, operator_bytecode)
            },
            Expr::FunctionCall {ref function, ref args, ..} => {
                let arg_load = itertools::join(args.iter().map(|x| x.generate_bytecode(context)), "\n");
            ...
            Expr::Index{ref base, ref slices} => {
                ...
                let t_size = context.type_map.get(&self.id).unwrap().size();
                let index_bytecode = match &slice {
                    // For now, ranges are not possible.
                    (Some(start), None, None) => {
                        start.generate_bytecode(context)
                    },
                    _ => panic!()
                };
                let base_bytecode = base.generate_bytecode(context);
            ...
            Expr::Int(ref int_lit) => {
                let t = context.type_map.get(&self.id).unwrap();
            ...
            Expr::VecLiteral(ref exprs) => {
                ...
                for (i, expr) in exprs.iter().enumerate() {
                    let value = expr.generate_bytecode(context);
            ...
            Expr::StructLiteral{ref base, ref fields} => {
                let args = itertools::join(fields.iter().map(|x| x.generate_bytecode(context)), "\n");
            ...
            Expr::AttributeAccess{ref base, ref attribute} => {
                let pointer_code = base.generate_bytecode(context);
                let t = context.type_map.get(&base.id).unwrap();
                let return_t = context.type_map.get(&self.id).unwrap();
...
impl ToBytecode for ComparisonOperator {
    fn generate_bytecode(&self, _context: &Context2) -> String {
        return match self {
...
mod tests {
    ...
    mod statements {
        use super::*;
            ...
            fn test_generate_assignment() {
                let (stmt, context) = compiler_layers::to_type_rewrites::<Node<Block>>("let foo = 2\nfoo = 3".as_bytes());
                let bytecode = stmt.generate_bytecode(&context);
        ...
        fn test_struct_declaration() {
            let input = "struct A:\n a: i32\n b: i32\n".as_bytes();
            let (_, _, bytecode) = compiler_layers::to_bytecode::<Node<Module>>(input);
    ...
    mod exprs {
        ...
        fn test_generate_array() {
            ...
            let (_stmt, _context, bytecode) = compiler_layers::to_bytecode::<Node<Stmt>>(input);
        ...
        fn test_array_access() {
            ...
            let (_stmt, _context, bytecode) = compiler_layers::to_bytecode::<Node<Block>>(input);
        ...
        fn test_struct_literal() {
            ...
            let (_stmt, _context, bytecode) = compiler_layers::to_bytecode::<Node<Module>>(input);
            let expected = format!("{}{}\n\
            (func $B   (local $x i32)\n\
            i32.const 1\n\
            i32.const 2\n\
        ...
        mod ops {
            ...
            fn test_generate_add() {
                let (add_expr, context) = compiler_layers::to_type_rewrites::<Node<Expr>>("5 + 6".as_bytes());
                let bytecode = add_expr.generate_bytecode(&context);
                assert_eq!(bytecode, "i32.const 5\ni32.const 6\ni32.add".to_string());
                let (add_expr, context) = compiler_layers::to_type_rewrites::<Node<Expr>>("5.0 + 6".as_bytes());
                let bytecode = add_expr.generate_bytecode(&context);
                assert_eq!(bytecode, "f32.const 5.0\nf64.promote/f32\ni32.const 6\nf64.convert_s/i32\nf64.add".to_string());
                let (add_expr, context) = compiler_layers::to_type_rewrites::<Node<Expr>>("5 + 6.0".as_bytes());
                let bytecode = add_expr.generate_bytecode(&context);
                assert_eq!(bytecode, "i32.const 5\nf64.convert_s/i32\nf32.const 6.0\nf64.promote/f32\nf64.add".to_string());
                let (add_expr, context) = compiler_layers::to_type_rewrites::<Node<Expr>>("5.0 + 6.0".as_bytes());
                let bytecode = add_expr.generate_bytecode(&context);
                assert_eq!(bytecode, "f32.const 5.0\nf32.const 6.0\nf32.add".to_string());
            }
            ...
            fn test_generate_subtract() {
                let (sub_expr, context) = compiler_layers::to_type_rewrites::<Node<Expr>>("1 - 2".as_bytes());
                let bytecode = sub_expr.generate_bytecode(&context);
                assert_eq!(bytecode, "i32.const 1\ni32.const 2\ni32.sub".to_string());
            }

            #[test]
            fn test_generate_multiply() {
                let (mult_expr, context) = compiler_layers::to_type_rewrites::<Node<Expr>>("3 * 4".as_bytes());
                let bytecode = mult_expr.generate_bytecode(&context);
                assert_eq!(bytecode, "i32.const 3\ni32.const 4\ni32.mul".to_string());
            }

            #[test]
            fn test_generate_divide() {
                let (div_expr, context) = compiler_layers::to_type_rewrites::<Node<Expr>>("8 / 9".as_bytes());
                let bytecode = div_expr.generate_bytecode(&context);
                assert_eq!(bytecode, "i32.const 8\nf64.convert_s/i32\ni32.const 9\nf64.convert_s/i32\nf64.div".to_string());
            }

            #[test]
            fn test_generate_rem() {
                let (rem_expr, context) = compiler_layers::to_type_rewrites::<Node<Expr>>("5 % 6".as_bytes());
                let bytecode = rem_expr.generate_bytecode(&context);
                assert_eq!(bytecode, "i32.const 5\ni32.const 6\ni32.rem_u".to_string());
            }
        }
    ...
    pub fn test_generate_function_call() {
        let (function_call, context) = compiler_layers::to_type_rewrites::<Node<Block>>(
            "fn a(b:i32) -> i32:\n return 1\nlet x = a(1)".as_bytes());
        let bytecode = "(func $a (param $b i32) (result i32) \ni32.const 1\n)\n(export \"a\" (func $a))\ni32.const 1\ncall $a\nset_local $x".to_string();
        assert_eq!(function_call.generate_bytecode(&context), bytecode);
    }
    ...
    fn test_generate_module() {
       ...
       assert_eq!(module.generate_bytecode(&context), expected);
   }

    #[test]
    fn test_generate_function() {
        let (func_stmt, context) = 
        compiler_layers::to_type_rewrites::<Node<Stmt>>("fn a(b: i32) -> i32:\n let x = 5 + 6\n return x\n".as_bytes());
        let bytecode = func_stmt.generate_bytecode(&context);
